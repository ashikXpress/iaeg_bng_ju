...
class HomeController extends Controller
{
    ...
    public function event($id){
        ...
        $events=Event::where('event_category_id',$id)->paginate(1);
    ...
    public function gallery($id)
    {
        ...
        $galleries=Gallery::where('category_id',$id)->paginate(1);
    ...
    public function member($id)
    {
        if($id==5){
            ...
            $members=Member::where('is_member',0)->where('status',1)->paginate(3);

        }else{
            $member_type=MemberType::findOrFail($id);
            $members=Member::where('is_member',1)->where('member_type_id',$id)->where('status',1)->paginate(1);
